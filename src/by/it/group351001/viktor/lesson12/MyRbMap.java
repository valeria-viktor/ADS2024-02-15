package by.it.group351001.viktor.lesson12;
...
public class MyRbMap implements SortedMap<Integer, String> {
    ...
    // Перечисление для представления цвета узла в красно-черном дереве
    private enum COLOR {
        BLACK,
        RED
    }

    // Внутренний класс для представления узла в красно-черном дереве
    private class TreeNode {
        TreeNode left; // Ссылка на левый дочерний узел
        TreeNode right; // Ссылка на правый дочерний узел
        TreeNode parent; // Ссылка на родительский узел
        Integer key; // Ключ узла
        String value; // Значение узла
        COLOR color; // Цвет узла

        // Конструктор для создания узла с заданными значениями
    ...
    private TreeNode root = null; // Корень дерева
    private int _size = 0; // Количество элементов в дереве

    // Метод для очистки дерева, рекурсивно удаляя все узлы
    ...
    private void rotateRight(TreeNode node) {
        TreeNode leftChild = node.left; //Сохраняем ссылку на левый узел, которое станет новым корнем поддерева после поворота.
        node.left = leftChild.right; //Перемещаем правое поддерево leftChild на место левого дочернего узла node.
        if (leftChild.right != null) { // есть ли у leftChild правый дочерний узел. Если он существует, нужно обновить его родителя.
